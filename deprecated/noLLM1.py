...
def fetch_jobs_sync(status: Optional[str] = None):
    with DB_LOCK:
        db = db_connect()
        if status:
            cur = db.execute("SELECT vod_id, streamer, title, status, progress, file_path, stream_url, error FROM jobs WHERE status = ? ORDER BY created_at", (status,))
        else:
            cur = db.execute("SELECT vod_id, streamer, title, status, progress, file_path, stream_url, error FROM jobs ORDER BY created_at")
        rows = cur.fetchall()
        db.close()
    keys = ["vod_id","streamer","title","status","progress","file_path","stream_url","error"]
    ...
    while True:
        try:
            ...
            for job in queued:
                ...
                duration = getattr(job, "duration", "01:00:00")  # fallback 1 hour if missing
